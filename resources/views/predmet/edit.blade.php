...
<div class="dodaj-predmet-middle">
    <div class="dodaj-predmet-title">IZMENI PREDMET</div>
    <form method="POST" action="{{ route('predmet.update', $predmet->id) }}" class="dodaj-predmet-form-wrap">
        @csrf
        @method('PUT')
        @if ($errors->any())
        ...
        <div class="dodaj-predmet-row">
            ...
            <input id="broj" name="broj" class="dodaj-predmet-input" type="text" value="{{ old('broj', $predmet->broj ?? '') }}" />
        </div>
        <div class="dodaj-predmet-row">
            ...
            <select id="vrsta" name="vrsta" class="dodaj-predmet-select">
                ...
                <option value="parnica" @selected(old('vrsta', $predmet->vrsta ?? '') === 'parnica')>Parnični predmet</option>
                <option value="krivicni" @selected(old('vrsta', $predmet->vrsta ?? '') === 'krivicni')>Krivični predmet</option>
            </select>
        </div>
        <div class="dodaj-predmet-row">
            <label class="dodaj-predmet-form-label" for="sud">Sud:</label>
            <input id="sud" name="sud" class="dodaj-predmet-input" type="text" value="{{ old('sud', $predmet->sud ?? '') }}" />
        </div>
        <div class="dodaj-predmet-row">
            <label class="dodaj-predmet-form-label" for="datum_prijema">Datum prijema:</label>
            <input id="datum_prijema" name="datum_prijema" class="dodaj-predmet-input dodaj-predmet-date" type="date" value="{{ old('datum_prijema', $predmet->datum_prijema ?? '') }}" />
        </div>
        <div class="dodaj-predmet-row">
            <label class="dodaj-predmet-form-label" for="rok">Rok:</label>
            <input id="rok" name="rok" class="dodaj-predmet-input dodaj-predmet-date" type="date" value="{{ old('rok', $predmet->rok ?? '') }}" />
        </div>
        <div class="dodaj-predmet-row">
            <label class="dodaj-predmet-form-label" for="status">Status:</label>
            <select id="status" name="status" class="dodaj-predmet-select">
                <option value="novo" @selected(old('status', $predmet->status ?? '') === 'novo')>Novo</option>
                <option value="u_obradi" @selected(old('status', $predmet->status ?? '') === 'u_obradi')>U obradi</option>
                <option value="zavrsen" @selected(old('status', $predmet->status ?? '') === 'zavrsen')>Završen</option>
                <option value="odbijen" @selected(old('status', $predmet->status ?? '') === 'odbijen')>Odbijen</option>
            </select>
        </div>
