...
<style>
    ...
    .sidebar-links .nav-link:nth-child(2) {
        background: #366185;
        color: #fff;
...
<div class="sidebar-custom">
    ...
    <div class="sidebar-links">
        ...
        <a href="{{ route('predmeti') }}" class="nav-link active">PREDMETI</a>
...
<div class="main-section">
    <div class="predmeti-content-middle">
        ...
        <div class="predmeti-actions-row">
            <input type="text" class="predmeti-search-input" placeholder="🔍 Pretraga predmeta">
            ...
            @can('predmet.create')
                <a href="{{ route('predmet.create') }}" class="predmeti-add-btn" style="margin-left:auto;">Dodaj predmet</a>
            @endcan
        </div>
        <div class="predmeti-table-wrap">
            <div class="custom-table p-2">
                <table class="table mb-0">
                    <thead>
                        <tr>
                            <th>Broj predmeta</th><th>Vrsta predmeta</th><th>Sud</th><th>Rok</th><th>Status</th><th>Opcije</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse(($predmets ?? []) as $predmet)
                            <tr>
                                <td>{{ $predmet->broj ?? '' }}</td>
                                <td>{{ $predmet->vrsta ?? '' }}</td>
                                <td>{{ $predmet->sud ?? '' }}</td>
                                ...
                                <td>
                                    @can('predmet.update')
                                        <a href="{{ route('predmet.edit', $predmet->id) }}" class="predmeti-btn-edit">Izmeni</a>
                                    @endcan
                                    @can('predmet.delete')
                                        <form method="POST" action="{{ route('predmet.destroy', $predmet->id) }}" style="display:inline-block;">
                                            @csrf
                                            @method('DELETE')
