...
<style>
    ...
    .sidebar-links .nav-link:nth-child(4) {
        background: #366185;
        color: #fff;
...
<div class="sidebar-custom">
    ...
    <div class="sidebar-links">
        ...
        <a href="{{ route('izvestaji') }}" class="nav-link active">IZVEŠTAJI</a>
    </div>
...
<div class="main-section">
    <div class="predmeti-content-middle">
        ...
        <div class="predmeti-actions-row">
            <input type="text" class="predmeti-search-input" placeholder="🔍 Pretraži izveštaje">
            ...
            @can('izvestaj.create')
                <a href="{{ route('izvestaj.create') }}" class="predmeti-add-btn" style="margin-left:auto;">Generiši novi izveštaj</a>
            @endcan
        </div>
        <div class="predmeti-table-wrap">
            <div class="custom-table p-2">
                <table class="table mb-0">
                    ...
                    <tbody>
                        @forelse(($izvestajs ?? []) as $izvestaj)
                            <tr>
                                <td>{{ $izvestaj->naziv ?? '' }}</td>
                                <td>{{ \Illuminate\Support\Str::limit($izvestaj->sadrzaj ?? '', 60, '...') }}</td>
                                <td>{{ $izvestaj->datum_kreiranja ?? ($izvestaj->datum ?? '') }}</td>
                                <td>{{ $izvestaj->pdf_putanja ?? '' }}</td>
                                <td>{{ $izvestaj->status ?? '' }}</td>
                                <td>
                                    @can('izvestaj.update')
                                        <a href="{{ route('izvestaj.edit', $izvestaj->id) }}" class="predmeti-btn-edit">Izmeni</a>
                                    @endcan
                                    @can('izvestaj.delete')
                                        <form method="POST" action="{{ route('izvestaj.destroy', $izvestaj->id) }}" style="display:inline-block;">
                                            @csrf
                                            @method('DELETE')
    ...
    (function () {
        ...
        const applyFilter = () => {
            ...
            Array.from(table.querySelectorAll('tbody tr')).forEach((tr) => {
                ...
                const statusText = statusIndex >= 0 ? ((tr.children[statusIndex]?.textContent || '').trim().toLowerCase()) : '';
